...
<head>
	...
	$(function() {	
	    ...
	    $('#dataList').datagrid({ 
	        ...
	        fit: true,//自动大小 
	        method: "post",
	        url:"BookServlet?method=getBookList&t="+new Date().getTime(),
	        idField:'id', 
	        ...
	        columns: [[  
				...
				{field:'free_number',title:'可借数量',width:70},
							...
							$.ajax({
								...
								success: function(data){
									if(data.type == "success"){
										...
										$("#add_book_category").combobox('setValue', "");
										$("#add_status").textbox('setValue', "");
										$("#add_info").val("");
										$("#add_number").numberbox('setValue', "");
										//重新刷新页面数据
							  			$('#dataList').datagrid("reload");
	  ...
	  //设置借阅图书窗口
	    $("#borrowDialog").dialog({
			...
			onBeforeOpen: function(){
				...
				$("#borrow_number").numberbox('setValue', selectRow.free_number);
				$("#real_borrow_number").numberbox({'max':selectRow.free_number});
			}
	    });
...
function uploadPhoto(){
	...
	var pos = action.indexOf('pid');
	if(pos != -1){
		action = action.substring(0,pos-1);
	}
	$("#uploadForm").attr('action',action+'&pid='+$("#set-photo-id").val());
	$("#uploadForm").submit();
}
	</script>
</head>
<body>
	...
	<div id="addDialog" style="padding: 10px">  
    	<form id="addForm" method="post">
	    	<table cellpadding="8" >
	    		<tr>
	    			<td>图书名称:</td>
	    			<td><input id="add_name" style="width: 200px; height: 30px;" class="easyui-textbox" type="text" name="name" data-options="required:true, missingMessage:'请填写图书名称'" /></td>
	    		</tr>
	    		<tr>
	    			<td>图书分类:</td>
	    			<td><input id="add_book_category" style="width: 200px; height: 30px;" class="easyui-textbox" type="text" name="bookCategoryId" data-options="required:true, missingMessage:'请填写图书分类'" /></td>
	    		</tr>
	    		<tr>
	    			<td>图书状态:</td>
	    			<td>
	    				<select id="add_status" style="width: 200px; height: 30px;" class="easyui-combobox" type="text" name="status" data-options="required:true, missingMessage:'请填写图书状态'">
	    		...
	    		<tr>
	    			<td>图书数量:</td>
	    			<td><input id="add_number" style="width: 200px; height: 30px;" class="easyui-numberbox" type="text" name="number" data-options="min:0,precision:0,required:true, missingMessage:'请填写图书数量'" /></td>
	    		</tr>
	    		<tr>
	    			<td>图书介绍:</td>
	...
	<div id="editDialog" style="padding: 10px">
    	<div style="float: right; margin: 20px 20px 0 0; width: 200px; border: 1px solid #EBF3FF">
	    	<img id="edit_photo" alt="照片" style="max-width: 200px; max-height: 400px;" title="照片" src="PhotoServlet?method=getPhoto" />
	    	<form id="uploadForm"  method="post" enctype="multipart/form-data" action="PhotoServlet?method=setPhoto">
	    		<input type="hidden" name="pid" id="set-photo-id">
    	...
    	<form id="editForm" method="post">
	    	...
	    	<table cellpadding="8" >
	    		<tr>
	    			<td>图书名称:</td>
	    			<td><input id="edit_name" style="width: 200px; height: 30px;" class="easyui-textbox" type="text" name="name" data-options="required:true, missingMessage:'请填写图书名称'" /></td>
	    		</tr>
	    		<tr>
	    			<td>图书分类:</td>
	    			<td><input id="edit_book_category" style="width: 200px; height: 30px;" class="easyui-textbox" type="text" name="bookCategoryId" data-options="required:true, missingMessage:'请填写图书名称'" /></td>
	    		</tr>
	    		<tr>
	    			<td>图书状态:</td>
	    			<td>
	    				<select id="edit_name" style="width: 200px; height: 30px;" class="easyui-combobox" type="text" name="status" data-options="required:true, missingMessage:'请填写图书名称'">
	    					<option value="1">可借</option>
	    					<option value="2">已借出</option>
	    					<option value="0">丢失或状态不可用</option>
	    				</select>
	    		...
	    		<tr>
	    			<td>图书数量:</td>
	    			<td><input id="edit_number" style="width: 200px; height: 30px;" class="easyui-numberbox" type="text" name="number" data-options="min:0,precision:0,required:true, missingMessage:'请填写图书数量'" /></td>
	    		</tr>
	    		<tr>
	    			<td>图书介绍:</td>
