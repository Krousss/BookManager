...
<head>
	...
	$(function() {	
	    ...
	    $('#dataList').datagrid({ 
	        ...
	        fit: true,//自动大小 
	        method: "post",
	        url : "BookCategoryServlet?method=getBookCategoryList&t=" + new Date().getTime(),
	        idField:'id', 
	    ...
	    $("#delete").click(function(){
        	...
        	if(selectLength == 0){
            	...
            	$.messager.confirm("消息提醒", "是否删除该分类?", function(r){
            		if(r){
            			$.ajax({
							type: "post",
							url: "BookCategoryServlet?method=deleteBookCategory",
							data: {ids: ids},
							dataType:'json',
	    	...
	    	buttons: [
	    		{
					...
					handler:function(){
						...
						if(!validate){
							...
							$.ajax({
								type: "post",
								url: "BookCategoryServlet?method=addBookCategory",
	    	...
	    	buttons: [
	    		{
					...
					handler:function(){
						...
						if(!validate){
							...
							$.ajax({
								type: "post",
								url: "BookCategoryServlet?method=editBookCategory&t="+new Date().getTime(),
...
<body>
	...
	<!-- 工具栏 -->
		<div id="toolbar">
		<c:if test="${user.type == 1||user.type==3}">
		...
		<div style="margin-left: 10px;">
			分类名：<input id="search-name" class="easyui-textbox" /><a
			...
			<table cellpadding="8">
				<tr>
					...
					<td><input id="add_name" class="easyui-textbox"
						...
						data-options="required:true, validType:'repeat', missingMessage:'请输入类名'" />
					</td>
				</tr>
			</table>
		</form>
